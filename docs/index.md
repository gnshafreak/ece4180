...
**Future work** would include using a microcontroller with more processing power and integrating the different functionalities. We could also implement a faster frequency extraction method to decrease the computational load on the processor. Additionally, we could create mechanical casing to create dsitance between the speaker and the microphone to help avoid feedback and increase ease of use. There is still room for improvmenet on the uLCD interface; we could give more information on how out of tune a note is and improve the graphics. We could also add more switches or digital sensors to give control over all 12 tones. 
